...
[dishoom, pizza_east, berliner_kebab, mac_do, mangia_buono].each do |attributes|
  ...
  review = Review.new(content: "I love #{restaurant.name}", rating: (3..5).to_a.sample)
  review.restaurant = restaurant
  review.save
  review2 = Review.new(content: "I hate #{restaurant.name}", rating: (0..2).to_a.sample)
  review2.restaurant = restaurant
  review2.save
  puts "Created 2 reviews for #{restaurant.name}"
end
